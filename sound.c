...
//#ifndef NICKY_DEBUG
    ...
    */
    ...
    if (song >= 0 && song < MAX_SND_MODULES)
    {
        ...
        // start new music
        Mix_Music* newsong = _songs[song];
        if (newsong != NULL)
        {
            Mix_PlayMusic(newsong, -1);
            Mix_VolumeMusic(200);           // cosmito : using volume of 200 since mod play volume seems low. This needs a hacked sdlmixer version (max vol is 128).
        }
    }
}
