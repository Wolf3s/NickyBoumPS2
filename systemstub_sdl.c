...
#include "mixer.h"
#include "sound.h"      // for MAX_SND_MODULES
#include "scaler.h"
...
static void setup_screen_surface(int fullscreen, int scaler) {
    ...
    screen_surface = SDL_SetVideoMode(w, h, 16, /*SDL_FULLSCREEN|*/SDL_HWSURFACE );
	if (!screen_surface) {
		print_error("!Unable to allocate screen_surface");
	}
...
void sys_init(int w, int h, const char *title) {
    ...
    SDL_Init(SDL_INIT_AUDIO|SDL_INIT_VIDEO|SDL_INIT_JOYSTICK|SDL_INIT_TIMER);       // cosmito : added SDL_INIT_AUDIOfor sdlmixer mod play
    ...
    if (_sfxMute)
    {
        ...
        {            
            ...
            for (i=0; i<MAX_SND_MODULES; i++)
            {
                _songs[i] = NULL;

                if (NICKY1) {
                    module_name = snd_module_table__v1[i];
                }
                if (NICKY2) {
                    module_name = snd_module_table__v2[i];
                }

                if (module_name != 0)
                {
                    sprintf(filename, "%s/%s", data_directory, module_name);
                    ...
                    fseek(fp, 0, SEEK_END);
                    int sample_size = ftell(fp);
                    fseek(fp, 0, SEEK_SET);
                    char *sample = (char *)malloc(sample_size);
                    sample_size = fread(sample, 1, sample_size, fp);
                    fclose(fp);
                    rw = SDL_RWFromMem(sample, sample_size);
                    _songs[i] = Mix_LoadMUS_RW(rw);
                    if (_songs[i] == NULL)
                    {
                        printf("Mix_LoadMUS_RW failed\n");
                        _sfxMute = 0;
                        break;
                    }
                }
        ...
        {
            _sfxMute = 0;   // revert to sfx mode if error.
        }   
    }
...
static void update_screen_from_surface(const uint8 *src) {
    ...
    SDL_Flip(screen_surface);       // se SDL_USE_HW_SURFACE=1 no Makefile do SDL chamar SDL_Flip em vez de SDL_UpdateRect
    // see http://sdl.beuc.net/sdl.wiki/SDL_Flip: The SDL_DOUBLEBUF flag must have been passed to SDL_SetVideoMode, when setting the video mode for this function to perform hardware flipping. 
#endif
}
        ...
        while (SDL_PollEvent(&ev))
        {
            ...
            case SDL_JOYBUTTONUP:
                if(ev.jbutton.button==1)
                    key_mask &= ~SKM_ACTION;
                else  if(ev.jbutton.button==6)  // 'L1' PS2 button
                    key_mask &= ~SKM_F2;        // cheat : next level
                else  if(ev.jbutton.button==8)  // 'L2' PS2 button
                    key_mask &= ~SKM_F3;        
                else  if(ev.jbutton.button==7)  // 'R1' PS2 button
                    key_mask &= ~SKM_F1;        // cheat : adding all bonuses
            ...
            case SDL_JOYAXISMOTION:
                if(ev.jaxis.axis == 0)
                {
                    ...
                    if (ev.jaxis.value > -JOYDEADZONE && ev.jaxis.value < JOYDEADZONE)
                    {
                        key_mask |= SKM_LEFT;
                        key_mask |= SKM_RIGHT;
                    }
